...
class gaussian_input_generator(object):
    ...
    def __init__(self, molecule, workflow_type, directory, theory, solvent, light_basis_set,
        ...
        if workflow_type == "equilibrium":
            self.tasks = (
                f"opt {theory}/{basis_set} {solvent_input}scf=(xqc,tight) integral(grid=ultrafinegrid)",
                f"freq {theory}/{basis_set} {solvent_input}volume NMR pop=NPA density=current Geom=AllCheck Guess=Read integral(grid=ultrafinegrid) scf(xqc,tight)",
                f"TD(NStates=5, Root=1) {theory}/{basis_set} {solvent_input}volume pop=NPA density=current Geom=AllCheck Guess=Read"
            )
    ...
    def _generate_file(self, tasks, name, resource_block, coords_block, charge, multiplicity) -> None:
        ...
        file_path = f"{self.directory}/{name}.com"
