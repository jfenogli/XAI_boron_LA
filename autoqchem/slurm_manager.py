...
class slurm_manager(object):
    ...
    def __init__(self, user, host, file_name, port):
        ...
        self.workdir = appdirs.user_data_dir(appauthor="autoqchem", appname=host.split('.')[0]) + f"/{file_name}/"
        ...
        self.remote_dir = f"/data/{self.user}/{file_name}/"
    ...
    def connect(self) -> None:
        ...
        if create_new_connection:            
            self.connection = ssh_connect(self.host, self.user, self.port)
    ...
    def create_jobs_for_molecule(self,
       ...
       #     return
        ...
        # create slurm files
        for com_file in glob.glob(f"{molecule_workdir}/*.com"):

            base_name = os.path.basename(os.path.splitext(com_file)[0])
            self._create_slurm_file_from_gaussian_file(base_name, molecule_workdir, wall_time, partition, n_processors)
            ...
            job = slurm_job(can=molecule.can,
                            ...
                            remote_dir = os.path.join(self.remote_dir, molecule.inchikey),
    ...
    def submit_jobs_from_jobs_dict(self, jobs) -> None:
        ...
        if jobs:
            ...
            for name, job in jobs.items():
                # copy .sh and .com file to remote_dir
                self.connection.run(f"mkdir -p {job.remote_dir}")
                self.connection.put(f"{job.directory}/{job.base_name}.sh", job.remote_dir)
                self.connection.put(f"{job.directory}/{job.base_name}.com", job.remote_dir)

                with self.connection.cd(job.remote_dir):
                    ret = self.connection.run(f"\n sbatch {job.base_name}.sh")
    ...
    def _retrieve_single_job(self, job) -> slurm_status:
        ...
        try:  # try to fetch the file
            log_file = self.connection.get(f"{job.remote_dir}/{job.base_name}.log",
    ...
    def resubmit_incomplete_jobs(self, wall_time="20:00:00") -> None:
        ...
        for key, job in incomplete_jobs.items():
            ...
            #    file_string = f.read()
            #old_wall_time = re.search(f"#SBATCH --time \d\d:\d\d:\d\d", file_string).group(1)
    ...
    def remove_jobs(self, jobs) -> None:
        ...
        for name, job in jobs.items():
            ...
            try : 
                ...
                print("could not find .sh local file")
            try : 
                os.remove(f"{job.directory}/{job.base_name}.com")
            except : 
                print("could not find .com local file") # gaussian file
            if os.path.exists(f"{job.directory}/{job.base_name}.log"):
                os.remove(f"{job.directory}/{job.base_name}.log")  # log file
            try :
                os.rmdir(f"{job.directory}")
            except :
                print("could not find local directory")
            # remove remote files
            try : self.connection.run(f"rm -r {job.remote_dir}")
            except :
                print("could not find remote directory for the job")
            #/slurm-{job.job_id}.out")
    ...
    def _create_slurm_file_from_gaussian_file(self, base_name, directory, wall_time, partition, n_processors) -> None:
        ...
        output = ""
        output += f"#!/bin/bash\n"
        output += f"#SBATCH --export=ALL,MODULEHOME=''\n" \
    ...
    def resubmit_failed_jobs() -> None:
        ...
        for key, job in jobs_failed.items():
            job_log = f"{job.directory}/{job.base_name}.log"
            job_com = f"{job.directory}/{job.base_name}.com"
        ...
        sm.submit_jobs_from_jobs_dict(jobs_failed)
