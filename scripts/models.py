...
dict_models = {
    'Linear' : {'fingerprints' : LinearRegression(), 'rdkit': LinearRegression() , 
            'quantum' : LinearRegression(), 'sigman': LinearRegression()},
    
    "Ridge" : {'fingerprints' : Ridge(alpha=6, max_iter=None, random_state=1, tol=0.01, solver = 'saga'), 'rdkit': Ridge(alpha=5, max_iter=500, random_state=1, tol=0.0001, solver = 'sparse_cg') , 
            'quantum' : Ridge(alpha=5, max_iter=500, random_state=1, tol=0.0001, solver = 'sparse_cg'), 'sigman': Ridge(alpha=0.01, max_iter=None, random_state=1, tol=0.0001, solver = 'auto')},
    ...
    "Bayes. Ridge": {'fingerprints' : BayesianRidge(alpha_1=1e-07, alpha_2=0.0001, lambda_1=0.0001, lambda_2=1e-07,
            ...
            'quantum' : BayesianRidge(alpha_1=1e-07, alpha_2=1e-07, lambda_1=0.0001, lambda_2=0.0001,
              n_iter=300, tol=0.01), 'sigman': BayesianRidge(alpha_1=0.0001, alpha_2=1e-07, lambda_1=1e-07, lambda_2=0.0001,
              n_iter=300, tol=0.01)},
    ...
    "Lasso" :{'fingerprints' : Lasso(alpha=1, max_iter=1000, precompute=False, random_state=1,
      selection='random', tol=0.001), 'rdkit': Lasso(alpha=0.1, max_iter=10000, precompute=False, selection = 'cyclic', random_state=1, tol=0.0001), 
            'quantum' : Lasso(alpha=0.001, max_iter=1000000, precompute=True, selection = 'cyclic', random_state=1, tol=0.01), 'sigman': Lasso(alpha=0.1, max_iter=10000, precompute=False, selection = 'cyclic', random_state=1, tol=0.00001)},
    
    "SVR": {'fingerprints' : SVR(C=1.0, epsilon=1.0, kernel='linear', shrinking=False), 'rdkit': SVR(epsilon=0.01, kernel='linear', tol=0.0001) , 
            'quantum' : SVR(epsilon=0.01, kernel='linear', tol=0.0001), 'sigman': SVR(epsilon=0.01, kernel='linear', tol=0.0001)},
   ...
   "Tree" : {'fingerprints' :DecisionTreeRegressor(criterion='absolute_error', max_depth=10,
            ...
            'quantum' : DecisionTreeRegressor(random_state=1), 'sigman': DecisionTreeRegressor(criterion='poisson', max_depth=10, max_features=1.0,
                      random_state=1)},
 ...
 bootstrap = False, min_samples_split = 3) , 
            'quantum' : RandomForestRegressor(criterion='friedman_mse', max_depth=10, max_features=None,
                      n_estimators=200), 'sigman': RandomForestRegressor()#params par défaut les mieux
           }  ,
    
    'GB': {'fingerprints' : GradientBoostingRegressor(loss='huber', max_features='sqrt',
                                           max_leaf_nodes=5,
           ...
           'sigman': GradientBoostingRegressor(loss='squared_error', max_leaf_nodes=2, n_estimators=500,
    ...
    'GPR' : {'fingerprints' : GaussianProcessRegressor(alpha=1e-11, kernel=RBF(length_scale=1.0),
            ...
            'quantum' : GaussianProcessRegressor(alpha=1e-09, kernel=RBF(length_scale=1),
                         normalize_y=True, random_state=1), 'sigman': GaussianProcessRegressor(alpha=1e-05, kernel=RBF(length_scale=1), n_restarts_optimizer = 0,
                         normalize_y=True)},
    
    'KNN' :{'fingerprints' : KNeighborsRegressor(p=1), 'rdkit': KNeighborsRegressor(n_neighbors=10, p=1, weights='distance') , 
            'quantum' : KNeighborsRegressor(n_neighbors=10, p=1, weights='distance'), 'sigman': KNeighborsRegressor(p=1, weights='distance')},
    ...
    'MLP' : {'fingerprints' : MLPRegressor(activation='logistic', alpha=0.001, hidden_layer_sizes=(100,),
            ...
            'sigman' : MLPRegressor(max_iter = 20000, hidden_layer_sizes=(100,), 
